...
def detect_red_object(frame):
    ...
    lower_red1 = np.array([18, 19, 0])
    upper_red1 = np.array([62, 108, 159])
    lower_red2 = np.array([18, 26, 53])
    upper_red2 = np.array([136, 142, 255])
...
cmd = "tutup gripper"
rospy.loginfo(f"Sending initial command: {cmd}")
pub.publish(cmd)
    ...
    if detected:
        ...
        if abs(center_x - frame_width // 2) < 20 and abs(center_y - camera_center_y) < 20 and not gripper_closed:
             if not gripper_closed:
                rospy.loginfo("Object is centered. Sending command: buka gripper")
